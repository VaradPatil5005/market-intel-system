...
def build_graph(metrics: MetricsCollector, entry_point: str = "ingest", simulate_failure_at: str | None = None):
    ...
    def node_rag(state: PipelineState) -> Dict[str, Any]:
        ...
        with get_session() as session:
            insights = rag_agent.run(session, state["run_id"], state["entities"], state["trend_results"])
    ...
    def node_forecast(state: PipelineState) -> Dict[str, Any]:
        """Tier 2 predictive layer. Non-fatal by design: forecasting is an
        enrichment, not a required step, so a failure here logs and moves
        on with an empty forecast list rather than halting the whole run."""
        try:
            _maybe_fail("forecast_trends")
            with get_session() as session:
                forecasts = forecasting_agent.run(session, state["run_id"], state["entities"])
    ...
    def node_human_review(state: PipelineState) -> Dict[str, Any]:
        with get_session() as session:
            insights = _all_insights(session, state["run_id"])
    ...
    graph.add_edge("score_confidence", "forecast_trends")
...
def _all_insights(session, run_id: str):
    """Insights for THIS run only — not the whole historical table.

    (Previously this pulled every insight ever created across every past
    run, which is why re-running the pipeline against the same DB made
    report lines like "Highest market-activity entities..." repeat once
    per prior run.)
    """
